...
std::tuple<double, double, double> Vision::FitEllipse(cv::Mat edge, bool drawEllipse, bool printEllipseInfoOnImage )
{
	...
	auto singleDropletContour = FindContoursOfSingleDroplet_AreaBased(edge); //TODO: Need a better way to identify the contour of interest. Processing only the ROI or use VISP ellipse tracker
	
	cv::RotatedRect RotRect = cv::fitEllipse(cv::Mat(singleDropletContour.outerEdge)); 
	//TODO: Using inner edge or average between inner and outer edges might be better than using outer edge.d
	//However Outer edge of the ellipse dissapears sometimes, causing the inner edge to be the new outer edge.
